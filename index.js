function Dundee(Type, methods, filter) {
  ...
  if (!methods) {
    methods = _requireExtensionFromNPM(Type);
  }
...
function _requireExtensionFromNPM (Type) {
  var npm_supported_objects = [String, Array, Object];
  
  if (npm_supported_objects.indexOf(Type) === -1) {
    throw new Error('No existing dundee package for ' + Type.name);
  }
  
  var extension_name = 'dundee-' + Type.name.toLowerCase();
  
  try {  
    return require(extension_name);
  } catch (error) {
    throw new Error('Add ' + extension_name + ' to dependencies in package.json');
  }
}
